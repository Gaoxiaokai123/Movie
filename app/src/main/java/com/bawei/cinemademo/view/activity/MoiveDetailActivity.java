...
public class MoiveDetailActivity extends BaseActivity {
    ...
    class DetailBack implements CallBackT<Detail> {
        @Override
        public void onSuccess(Detail data) {
            FrescoUtils.getInstance().showUrlBlur(detailImage, data.imageUrl, 1, 1);
            detailCommentNum.setText("评论   " + data.commentNum + "万条");
            detailScore.setText("评分   " + data.score + "分");
            detailName.setText(data.name);
            detailTypeAndtimes.setText(data.movieType + "   " + data.duration);
